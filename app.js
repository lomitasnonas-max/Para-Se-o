...
function doLogin() {
  ...
  const nombre = (typeof FRIEND_NAME !== 'undefined' && FRIEND_NAME) ? FRIEND_NAME : '';
  let saludo = 'esto lo hice pensando en ti 🩷';
  if (h >= 5  && h < 12) saludo = `buenos días ${nombre} ☀️`;
  else if (h >= 12 && h < 19) saludo = `buenas tardes ${nombre} 🌸`;
  else if (h >= 19 && h < 23) saludo = `buenas noches ${nombre} 🌙`;
...
// Páginas sin pantalla de login (main.html = acceso directo a la
// galería para visitas de regreso): mostrar todo de una vez, sin
// esperar clicks de login/bienvenida que aquí no existen.
if (!$('login')) {
  const mainEl = $('main');
  if (mainEl) mainEl.classList.add('show');
  renderGallery();
  setTimeout(() => { const mf = $('msg-fab'); if (mf) mf.classList.add('on'); }, 300);
  setTimeout(() => { const df = $('dedica-fab'); if (df) df.classList.add('on'); }, 500);
  setTimeout(() => { const ff = $('final-fab'); if (ff) ff.classList.add('on'); }, 1200);
  setTimeout(() => { const pl = $('playlist-fab-card'); if (pl) pl.classList.add('on'); }, 700);
}
...
function renderGallery() {
  ...
  sorted.forEach(({ c, i }, idx) => {
    ...
    const isKnight = c.cat === 'paulo';
    const corners = isKnight
      ? `<span class="card-corner corner-tl"></span><span class="card-corner corner-tr"></span><span class="card-corner corner-bl"></span><span class="card-corner corner-br"></span>`
      : '';
    const posAttr = c.pos ? ` style="object-position:${c.pos}"` : '';
    card.innerHTML = `
      <img src="${c.img}" alt="${displayName}"${posAttr}
      ...
      <div class="card-note">${isKnight ? '⚔️' : '🎵'}</div>
    `;
...
function openCard(card, i) {
  ...
  hdrBg.style.backgroundPosition = '';
  ...
  if (isOniChan) {
    ...
    $('modal-img').style.objectPosition = ch.pos || '';
    ...
    testImg.onload = () => {
      ...
      hdrBg.style.backgroundPosition = ch.pos || '';
    };
    testImg.onerror = () => {
  ...
  $('lyrics-admin').style.display = 'flex'; // buscador de letras (LRCLIB) visible para todos
  $('fetch-artist').value = ch.artist || '';
  ...
  if (cachedLyrics[i]) {
    ...
    fetchLyrics(ch.songName, ch.artist || '').then(lyrics => {
...
async function adminFetchLyrics() {
  ...
  const artist = $('fetch-artist').value || ch.artist || '';
...
function openDedica() {
  ...
  const dImg = (typeof DEDICA_IMG !== 'undefined' && DEDICA_IMG) ? DEDICA_IMG : '';
  if (dImg) {
    $('dedica-img').src = dImg;
    $('dedica-hdr-bg').style.backgroundImage = `url("${dImg}")`;
  }
  ...
  const dSong   = typeof DEDICA_SONG !== 'undefined' ? DEDICA_SONG : '';
  const dName   = typeof DEDICA_SONG_NAME !== 'undefined' ? DEDICA_SONG_NAME : '';
  const dArtist = typeof DEDICA_ARTIST !== 'undefined' ? DEDICA_ARTIST : '';
  dedicaAud.src = dSong;
  dedicaAud.volume = 0;
  ...
  setTimeout(() => {
    ...
    if (dName && dArtist) {
      fetchLyrics(dName, dArtist).then(lyrics => {
...
// INIT
{ const wNameEl = $('w-name'); if (wNameEl) wNameEl.textContent = FRIEND_NAME; }

// ════════════════════════════════════════════════════
//   PERSONALIZACIÓN AUTOMÁTICA DESDE data.js
//   (así mañana solo se edita data.js y todo se actualiza)
// ════════════════════════════════════════════════════
function applyPersonalization() {
  const setTxt = (id, val) => { const el = $(id); if (el && val) el.textContent = val; };
  const setSrc = (id, val) => { const el = $(id); if (el && val) el.src = val; };
  const setBg  = (id, val) => { const el = $(id); if (el && val) el.style.backgroundImage = `url("${val}")`; };
  // NOTA: los const de data.js NO quedan colgados de "window" en un script
  // clásico, así que aquí se referencian directo (no por nombre en texto).
  const _FRIEND_NAME    = typeof FRIEND_NAME    !== 'undefined' ? FRIEND_NAME    : '';
  const _DEDICA_SONG_NAME  = typeof DEDICA_SONG_NAME  !== 'undefined' ? DEDICA_SONG_NAME  : '';
  const _DEDICA_ARTIST     = typeof DEDICA_ARTIST     !== 'undefined' ? DEDICA_ARTIST     : '';
  const _DEDICA_IMG        = typeof DEDICA_IMG        !== 'undefined' ? DEDICA_IMG        : '';
  const _DEDICA2_SONG_NAME = typeof DEDICA2_SONG_NAME !== 'undefined' ? DEDICA2_SONG_NAME : '';
  const _DEDICA2_ARTIST    = typeof DEDICA2_ARTIST    !== 'undefined' ? DEDICA2_ARTIST    : '';
  const _DEDICA2_IMG       = typeof DEDICA2_IMG       !== 'undefined' ? DEDICA2_IMG       : '';
  const _DEDICA3_SONG_NAME = typeof DEDICA3_SONG_NAME !== 'undefined' ? DEDICA3_SONG_NAME : '';
  const _DEDICA3_ARTIST    = typeof DEDICA3_ARTIST    !== 'undefined' ? DEDICA3_ARTIST    : '';
  const _DEDICA3_IMG       = typeof DEDICA3_IMG       !== 'undefined' ? DEDICA3_IMG       : '';
  const _BDAY_PAULO_SONG_NAME = typeof BDAY_PAULO_SONG_NAME !== 'undefined' ? BDAY_PAULO_SONG_NAME : '';
  const _BDAY_PAULO_ARTIST    = typeof BDAY_PAULO_ARTIST    !== 'undefined' ? BDAY_PAULO_ARTIST    : '';
  const _BDAY_PAULO_BADGE     = typeof BDAY_PAULO_BADGE     !== 'undefined' ? BDAY_PAULO_BADGE     : '';
  const _BDAY_PAULO_IMG       = typeof BDAY_PAULO_IMG       !== 'undefined' ? BDAY_PAULO_IMG       : '';
  const _FRIENDSHIP_PHOTO         = typeof FRIENDSHIP_PHOTO         !== 'undefined' ? FRIENDSHIP_PHOTO         : '';
  const _FRIENDSHIP_PHOTO_CAPTION = typeof FRIENDSHIP_PHOTO_CAPTION !== 'undefined' ? FRIENDSHIP_PHOTO_CAPTION : '';

  if (_FRIEND_NAME) setTxt('login-guest-name', 'Soy ' + _FRIEND_NAME);

  // Dedicatoria principal
  setTxt('dedica-fab-song', _DEDICA_SONG_NAME);
  setTxt('dedica-fab-artist', _DEDICA_ARTIST);
  setTxt('dedica-song', _DEDICA_SONG_NAME);
  setTxt('dedica-artist', _DEDICA_ARTIST);
  if (_DEDICA_IMG) setSrc('dedica-fab-img', _DEDICA_IMG);

  // Canción especial 1
  setTxt('dedica2-fab-song', _DEDICA2_SONG_NAME);
  setTxt('dedica2-fab-artist', _DEDICA2_ARTIST);
  setTxt('dedica2-title', _DEDICA2_SONG_NAME);
  setTxt('dedica2-artist-txt', _DEDICA2_ARTIST);
  setTxt('dedica2-song-name', _DEDICA2_SONG_NAME);
  if (_DEDICA2_IMG) { setSrc('dedica2-fab-img', _DEDICA2_IMG); setBg('dedica2-hdr-bg', _DEDICA2_IMG); }

  // Canción especial 2
  setTxt('dedica3-fab-song', _DEDICA3_SONG_NAME);
  setTxt('dedica3-fab-artist', _DEDICA3_ARTIST);
  setTxt('dedica3-title', _DEDICA3_SONG_NAME);
  setTxt('dedica3-artist-txt', _DEDICA3_ARTIST);
  setTxt('dedica3-song-name', _DEDICA3_SONG_NAME);
  if (_DEDICA3_IMG) { setSrc('dedica3-fab-img', _DEDICA3_IMG); setBg('dedica3-hdr-bg', _DEDICA3_IMG); }

  // Canción sorpresa de cumpleaños
  setTxt('paulo-new-song', _BDAY_PAULO_SONG_NAME);
  setTxt('paulo-new-artist', _BDAY_PAULO_ARTIST);
  setTxt('paulo-new-badge', _BDAY_PAULO_BADGE);
  setTxt('paulo-new-title', _BDAY_PAULO_SONG_NAME);
  setTxt('paulo-new-sname', _BDAY_PAULO_SONG_NAME);
  if (_BDAY_PAULO_IMG) { setSrc('paulo-new-img', _BDAY_PAULO_IMG); setBg('paulo-new-hdr-bg', _BDAY_PAULO_IMG); }

  // Evidencia de la fecha de amistad
  if (_FRIENDSHIP_PHOTO) setSrc('fecha-img', _FRIENDSHIP_PHOTO);
  setTxt('fecha-caption-date', _FRIENDSHIP_PHOTO_CAPTION);
}
...
function renderBookPage(direction) {
  ...
  const vidEl = $('book-page-video');
  const txtEl = $('book-page-text');
  const dateEl = $('book-page-date');

  // Página con VIDEO en vez de foto
  if (page.video) {
    if (vidEl) {
      vidEl.pause();
      vidEl.src = page.video;
...
function winGame() {
  ...
  $('game-win-msg').textContent = `Te quiero mucho, ${typeof FRIEND_NAME !== 'undefined' ? FRIEND_NAME : ''} 🩷`;
...
async function openDedica3() {
  ...
  try {
    ...
    if(cache['awm']){ data=cache['awm']; }
      ...
      const _n3 = typeof DEDICA3_SONG_NAME !== 'undefined' ? DEDICA3_SONG_NAME : '';
      const _a3 = typeof DEDICA3_ARTIST !== 'undefined' ? DEDICA3_ARTIST : '';
      if (_n3 && _a3) data = await fetchLyrics(_n3, _a3);
...
async function searchDedica2Lyrics() {
  const lyricsEl = $('dedica2-lyrics');
  const track  = ($('d2-track')  && $('d2-track').value.trim())  || (typeof DEDICA2_SONG_NAME !== 'undefined' ? DEDICA2_SONG_NAME : '');
  const artist = ($('d2-artist') && $('d2-artist').value.trim()) || (typeof DEDICA2_ARTIST !== 'undefined' ? DEDICA2_ARTIST : '');
...
async function searchDedica3Lyrics() {
  const lyricsEl = $('dedica3-lyrics');
  const track  = ($('d3-track')  && $('d3-track').value.trim())  || (typeof DEDICA3_SONG_NAME !== 'undefined' ? DEDICA3_SONG_NAME : '');
  const artist = ($('d3-artist') && $('d3-artist').value.trim()) || (typeof DEDICA3_ARTIST !== 'undefined' ? DEDICA3_ARTIST : '');
  if (lyricsEl) lyricsEl.innerHTML = '<span style="color:var(--muted);font-size:.8rem">buscando...</span>';
...
// ════════════════════════════════════════════════════
//   CANCIÓN SORPRESA — esquina superior derecha
// ════════════════════════════════════════════════════
const pauloNewAud = $('paulo-new-aud');
if (pauloNewAud) pauloNewAud.volume = .85;

// Fab de "canción sorpresa" desactivado (se dejó de usar)
// (function() {
//   const obs = new MutationObserver(() => {
//     const main = $('main');
//     if (main && main.classList.contains('show')) {
